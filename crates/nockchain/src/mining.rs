...
pub fn init_mining_thread_pool() {
    ...
    info!("Initializing parallel mining thread pool with {} threads", threads);
    ...
    match pool_result {
        Ok(_) => info!("Successfully built global thread pool with {} threads", threads),
        Err(e) => info!("Failed to build thread pool (may already exist): {}", e),
    }
}
...
pub fn bpmul_parallel_jet(context: &mut Context, subject: Noun) -> Result {
    ...
    if res_len > 32 {  // Lowered from 64
        info!("bpmul using parallel FFT for size {}", res_len);
...
pub fn bp_hadamard_parallel_jet(context: &mut Context, subject: Noun) -> Result {
    ...
    if res_len > 256 && threads > 1 {  // Lowered from 1024
        info!("bp_hadamard using {} threads for size {}", threads, res_len);
...
fn bp_ntt_parallel(bp: &[Belt], root: &Belt) -> Vec<Belt> {
    ...
    let threads = get_mining_threads();
    info!("bp_ntt_parallel called with n={}, using {} threads", n, threads);
    
    if n >= 256 && threads > 1 {  // Lowered from 1024
        info!("Using parallel bit reversal for size {}", n);
    ...
    for stage in 0..log_n {
        ...
        if m >= 16 && threads > 1 {  // Lowered from 64
            info!("Stage {} using parallel butterflies (m={})", stage, m);
...
fn bpmul_fft_parallel(a: &[Belt], b: &[Belt], result: &mut [Belt]) -> std::result::Result<(), crate::form::math::FieldError> {
    ...
    let threads = get_mining_threads();
    if padded_len >= 256 && threads > 1 {  // Lowered from 1024
        info!("Parallel pointwise multiplication with {} threads for size {}", threads, padded_len);
